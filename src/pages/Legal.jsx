...
const H2 = 'mt-[clamp(40px,5vw,64px)] font-serif text-[clamp(23px,2.3vw,32px)] leading-[1.2] tracking-[-0.01em]';
const H3 = 'mt-6.5 text-[11px] uppercase tracking-[0.22em]';
...
export default function Legal({ doc }) {
  ...
  return (
    <div className="mx-auto max-w-[820px] px-[30px] pb-[clamp(70px,9vw,130px)] pt-[calc(clamp(70px,9vw,130px)+60px)]">
      <p className="mb-5.5 text-[11px] uppercase tracking-[0.32em] text-muted">{t(page.kickerIt, page.kickerEn)}</p>
      <h1 className="font-serif text-[clamp(34px,4.6vw,72px)] leading-none tracking-[-0.02em]">{t(page.it, page.en)}</h1>
      <p className="mt-5.5 text-[13px] leading-[1.7] text-muted">
      ...
      {page.blocks.map(([tag, content], i) => {
        ...
        if (tag === 'links')
          return (
            <ul key={i} className="mt-4 grid list-none gap-2.5 p-0 text-[14px] leading-[1.6]">
              {content.map(([label, href, text]) => (
                <li key={href} className="flex flex-wrap gap-2.5">
                  <span className="min-w-[110px] text-[10px] uppercase tracking-[0.2em] text-muted">{label}</span>
      ...
      <p className="mt-[clamp(44px,5vw,70px)] border-t border-line pt-6 text-[11px] uppercase tracking-[0.2em]">
